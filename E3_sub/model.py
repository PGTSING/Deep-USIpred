...
from utils import sample, emb, get_roc_score
import numpy as np
...
class ProtEncoder(nn.Module):
    def __init__(self):
        ...
        self.prot_fc1 = ProtConv(1, 16, 8, 0,dropout=0.25)
        self.prot_fc2 = ProtConv(16, 32, 8, 0,dropout=0.25)
        self.fc = Perceptron(1010, 768, act='relu',dropout=0.25)
...
class GNetNN(nn.Module):
    ...
    def forward(self, x):
        x_e3 = x[:,:1024]
        x_sub = x[:,1024:]
        x_e3 = self.drug_encoder(x_e3)
        x_sub = self.prot_encoder(x_sub)
        x = torch.cat([x_e3, x_sub], -1)
...
def Train():
    ...
    for i in range(len(train_index)):
        ...
        net = GNetNN()
        net.eval()
        checkpoint = torch.load("model_train/model_1/model_%s.pkl"%(str(fold_i)))
