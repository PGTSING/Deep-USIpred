...
def make_batch(args, data, idx):
    ...
    batch_drug_data = collate_prot_data(args, data=drug_subdata)
    batch_label_data = label_subdata

    # Cast tensor structure
    batch_prot_data = cast_tensor(batch_prot_data, dtype=['f'])
    batch_drug_data = cast_tensor(batch_drug_data, dtype=['f'])
...
def cast_cuda(data):
    assert isinstance(data, list)
    cast_data = [elem.to("cuda:1") for elem in data]
...
class MoleculeDataset(Dataset):
    def __init__(self, args):
        ...
        self.prot_fea_list1 = dataset['prot_fea1']
        self.prot_fea_list_test = dataset['prot_fea_test']
        self.prot_fea_list1_test = dataset['prot_fea1_test']
        ...
        self.label_list_test = dataset['label_test']
        self.split_list = dataset['split']
        self.split_list1 = dataset['split1']
        self.data = [self.prot_fea_list,
                     self.prot_fea_list1,
                     self.label_list]

        self.data_test = [self.prot_fea_list_test,
                     self.prot_fea_list1_test,
                     self.label_list_test]
        self.dim_prot_fea = len(self.prot_fea_list[0])
        self.dim_prot_fea1 = len(self.prot_fea_list1[0])
    ...
    def load_data(self, partition, add_noise=False):
        ...
        #     data = get_subdata(self.data, self.val_idx_list)
            ...
            data = get_subdata(self.data_test, self.test_idx_list)
            # print(Counter(data[-1]))
    ...
    def preprocess(self):
        ...
        # self.val_idx_list += list(self.split_list[-1])
        for i in range(n_split):
            self.train_idx_list += list(self.split_list[i])

        for i in range(n_split1):
            self.test_idx_list += list(self.split_list1[i])
